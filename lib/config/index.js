...
config.reloadAllUserList = function () {
  ...
  this.allUserList = [...this.gtrcUserGroupList];

  // { group: 'global', name: config.gitconfigData.user?.name, email: config.gitconfigData.user?.email }

  if (this.gitconfigData.user?.name) {
    this.allUserList.unshift({
      group: 'global',
      name: config.gitconfigData.user?.name,
      email: config.gitconfigData.user?.email
    });
  }
};
